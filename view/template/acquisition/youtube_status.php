        ...
        <div class="inner">
            <div class="content">
                <?php if (preg_match('/^[A-Za-z0-9._%+-]+@plusgoogle.com$/', $statusData['email'])): ?>
                    <div id="email-google-plus-error" class="error">Your email address is set as <?php echo $statusData['email']; ?>.<br>If this is not your
                        email address, please <span id="scroll_email">change it below</span>.
                    </div>
                <?php endif ?>
                ...
                <div class="confirmation-steps">
                    <ul>
                        ...
                        <li class="<?php echo $isRewardClaimed && $isSyncAgreed ? "" : "disabled" ?>">
                            ...
                            <p <?php echo ($isRewardClaimed === false && $isSyncAgreed === true) ? "" : "hidden" ?>>To get your credits, <a href="/get">download
                                    the app</a> and <a href="/faq/youtube">follow these instructions.</a></p>
                        </li>
                    </ul>
                </div>
            ...
            <div class="content">
                ...
                <form id="youtube_settings" action="/youtube/edit" method="post">
                    ...
                    <div class="block">
                        ...
                        <input type="text" id="channel-name" name="new_preferred_channel" placeholder="@YourPreferredChannelName"
                               value="<?php echo $statusData['lbry_channel_name']; ?>" <?php if ($statusData['status'] == 'syncing' || $statusData['status'] == 'synced'): echo "disabled"; endif; ?> >
                    ...
                    <div class="block">
                        <label for="fee-checkbox" class="block full">
                            <input type="checkbox" name="paid" id="fee-checkbox" <?php
                            echo ($statusData['fee'] === null) ? "" : "checked";
                            ?> onchange="displayFees()">I want to set a price for my videos
                        </label>
                        <div id="fee-settings" <?php echo $statusData['fee'] === null ? "hidden" : ""; ?>>
                            <label for="fee-amount">Amount</label>
                            <input type="text" id="fee-amount" name="fee_amount"
                                   value="<?php echo $statusData['fee'] !== null ? $statusData['fee']['amount'] : "0"; ?>" class="required-fields">
                            <label for="fee-currency">Currency</label>
                            <select id="fee-currency" name="fee_currency">
                                <option value="LBC" <?php echo $statusData['fee'] !== null && ['currency'] === 'LBC' ? "selected" : ""; ?>>LBC</option>
                                <option value="USD" <?php echo $statusData['fee'] !== null && $statusData['fee']['currency'] === 'USD' ? "selected" : ""; ?>>
                                    USD
                                </option>
                            </select>
                            <label for="fee-address">Address</label>
                            <input type="text" id="fee-address" name="fee_address" placeholder="b1234567890abcdefghijklmnopqrstuvw" autocomplete="new-password"
                                   class="required-fields"
                                   value="<?php echo $statusData['fee']['address']; ?>">
                        </div>
                    </div>
                    <label for="sync-consent" class="block full">
                        <input name="sync_consent" id="sync-consent"
                               type="checkbox" <?php if ($statusData['status'] == 'queued'): echo "checked"; endif; ?> <?php if ($statusData['status'] == 'syncing' || $statusData['status'] == 'synced'): echo "disabled ";
                            echo "checked"; endif; ?>>I want to sync my content to the LBRY network and agree to <a href="/faq/youtube-terms">these terms</a>.
